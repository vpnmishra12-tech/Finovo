...
import { ShieldCheck, Camera, Image as ImageIcon, Loader2, AlertTriangle, CheckCircle2, Search, Info } from 'lucide-react';
      ...
      <div className="bg-primary p-6 rounded-[2rem] text-white relative overflow-hidden shadow-2xl">
        ...
        <div className="relative z-10 space-y-2">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-2xl font-headline font-black uppercase tracking-tight">{t.agent.auditorTitle}</h1>
          </div>
          <p className="text-[10px] text-white/60 uppercase tracking-widest font-bold">{t.agent.auditorDesc}</p>
        </div>
      </div>
        ...
        <CardContent className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-1 gap-3">
            ...
            <Button
              ...
              disabled={isAuditing}
              className="h-12 rounded-xl font-black uppercase text-[10px] tracking-widest border-dashed border-2"
            >
              <ImageIcon className="w-4 h-4 mr-2" /> {t.agent.uploadButton}
            </Button>
          </div>
          ...
          <Card className={cn("border-none shadow-lg rounded-[1.5rem] overflow-hidden", auditResult.isCorrect ? "bg-green-50" : "bg-red-50")}>
            ...
            <CardContent className="space-y-4">
              <div className="bg-white/80 p-4 rounded-2xl border border-border/50 space-y-3">
                ...
                {!auditResult.isCorrect && auditResult.detectedErrors.length > 0 && (
                  <div className="space-y-2 pt-2">
                    <p className="text-[9px] font-black uppercase text-red-600 tracking-widest">Errors Found:</p>
                    <ul className="space-y-1">
                      {auditResult.detectedErrors.map((err, i) => (
                        <li key={i} className="text-[10px] bg-red-100/50 text-red-700 px-3 py-1.5 rounded-lg font-bold border border-red-200">
                          • {err}
                        </li>
                      ))}
                    </ul>
